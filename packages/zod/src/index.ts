...
export const userSignup = z.object({
    username : z.string().min(3).max(20,{message : "username should be minimum 3 words maximum 20 words"}),
...
export const userSignin = z.object({
    username : z.string().min(3).max(20, {message : "username must be min 3 words"}),
    password : z.string().min(6).max(20, {message : "password should be min 6 words"})
})
